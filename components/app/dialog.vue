...
<template>
  <div v-if="layer" class="fixed inset-0 z-40 overflow-y-auto" aria-labelledby="modal-title" role="dialog"
    aria-modal="true">
    <div class="block min-h-screen items-end justify-center p-0 text-center">
      <Transition>
        <div v-if="overlay" class="fixed inset-0 bg-gray-500 bg-opacity-75 transition-opacity" aria-hidden="true"></div>
      </Transition>
      <!-- This element is to trick the browser into centering the modal contents. -->
      <span class="hidden md:inline-block md:h-screen md:align-middle" aria-hidden="true">&#8203;</span>

      <Transition>
        <div v-if="modal" class="fixed inset-0 z-50 overflow-y-auto scrollbar-thin">
          <div class="md:min-h-32 block min-h-screen items-end justify-center p-0 text-center">
            <!-- This element is to trick the browser into centering the modal contents. -->
            <span class="hidden md:inline-block md:h-screen md:align-middle" aria-hidden="true">&#8203;</span>

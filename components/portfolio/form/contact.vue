        ...
        <div class="relative">
          ...
          <div class="max-w-xl pt-10 mx-auto">
            <form class="grid grid-cols-1 gap-y-6" @submit.prevent="submit">
              <div>
                <label for="full_name" class="sr-only">{{ "fields.name" }}*</label>
                <input id="name" v-model="form.name" type="text" name="name" autocomplete="name" maxlength="100"
                  ...
                  :placeholder="`${'fields.name'}*`" required />
              </div>
              <div>
                <label for="email" class="sr-only">{{ "fields.email" }}*</label>
                <input id="email" v-model="form.email" name="email" type="email" autocomplete="email" maxlength="100"
                  class="block w-full px-4 py-3 placeholder-gray-500 border-gray-300 rounded-md shadow-sm focus:ring-purple-500 focus:border-purple-500 dark:text-gray-900"
                  :placeholder="`${'fields.email'}*`" required />
              </div>
              <div>
                <label for="message" class="sr-only">{{ "fields.message" }}*</label>
                <textarea id="message" v-model="form.message" name="message" rows="4"
                  class="block w-full px-4 py-3 placeholder-gray-500 border-gray-300 rounded-md shadow-sm focus:ring-purple-500 focus:border-purple-500 dark:text-gray-900"
                  minlength="25" maxlength="1500" :placeholder="`${'fields.message'}*`" required />
              ...
              <div class="hidden">
                <div class="relative flex items-start">
                  ...
                  <div class="ml-3 text-sm">
                    <label for="conditions" class="font-medium text-gray-700">I accept conditions</label>
                  </div>
                </div>
              </div>

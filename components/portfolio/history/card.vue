...
<template>
  <div class="flex flex-col items-start flex-grow sm:items-center sm:flex-row">
    <div
      class="inline-flex items-center justify-center flex-shrink-0 w-24 h-24 text-primary-500 bg-gray-100 dark:bg-gray-800 rounded-full m-auto lg:mr-6"
    >
      <div class="h-16 w-16 flex">
        <svg-icon
          :name="`history-${historyItem.slug}`"
          class="formation-logo mx-auto h-16 w-16 text-primary-600"
          loading="lazy"
        />
      </div>
    </div>
    ...
    <div class="flex-grow pt-10 pb-6 pl-6">
      ...
      <span class="pt-1">
        <h3 class="font-header text-xl font-bold uppercase text-primary-600">
      ...
      <span
        ...
        class="text-xs bg-primary-600 bg-opacity-30 px-1 py-1 rounded-md font-semibold text-white flex items-center space-x-1 w-max mt-2"
      >
        <span class="bg-primary-600 w-2 h-2 block rounded-full" />
